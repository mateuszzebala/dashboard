...
export const Select = ({ data, value = null, setValue, emptyName = 'SELECT', asButton, label, canBeNull = true, size = 1, ...props }) => {
    ...
    if (asButton) {
        return (
            <Button size={size} {...props} onClick={handleOnClick}>
                {props.children}
            </Button>
        )
    }
    ...
    return (
        <StyledWrapper onClick={handleOnClick} size={size}>
            <StyledLabel size={size} label={toBoolStr(label)}>
                {label || ''}
            </StyledLabel>
            <StyledValue size={size}>{data[value] === null ? <span>{emptyName}</span> : data[value]}</StyledValue>
